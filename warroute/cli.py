...
def plan(
    ...
    stop: list[str] | None = typer.Option(  # noqa: B008
        ...
        "--stop",
        help=(
            "Multi-stop: LAT,LON or LAT,LON:DWELL[:overnight[:HHMM]]. Repeatable."
            " Implies oneway. HHMM = 24h deadline (e.g. 1600 = 'arrive by 4PM')."
        ),
    ),
    ...
    from warroute.router.planner import PlannerError, PlanRequest, Stop, parse_arrive_hhmm
    ...
    if stop:
        for raw in stop:
            try:
                ...
                arrive_by = parse_arrive_hhmm(parts[3]) if len(parts) > 3 else None
                stops.append(
                    Stop(
                        lat=float(lat_s),
                        lon=float(lon_s),
...
def precheck() -> None:
    ...
    Do NOT run from a network with TLS interception (e.g. a corporate/school
    Fortinet MITM proxy); tokens transit through the inspection device. See
