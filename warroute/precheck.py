...
are healthy *before* the operator turns the key on a planned wardrive. Cheap by design:
...
async def check_wdgowars_version() -> CheckResult:
    """Compare the live WDGoWars server version against the pinned expectation.

    Informational and non-fatal: a version bump may expose new endpoints worth
    re-probing (DECISIONS.md 2026-05-11), but it never blocks a drive. Any error
    or unknown version downgrades to WARN, not FAIL, since check_wdgowars already
    covers real auth/reachability.
    """
    expected = get_settings().wdgowars_expected_version
    try:
        async with WdgowarsClient() as wdg:
            live = await wdg.server_version()
    except WdgowarsError as exc:
        return CheckResult(
            name="WDGoWars version",
            status=Status.WARN,
            detail=f"could not read /api/stats: {exc}",
        )
    if live is None:
        return CheckResult(
            name="WDGoWars version",
            status=Status.WARN,
            detail="/api/stats returned no version field",
        )
    if live == expected:
        return CheckResult(
            name="WDGoWars version",
            status=Status.OK,
            detail=f"server v{live} matches pinned {expected}",
        )
    return CheckResult(
        name="WDGoWars version",
        status=Status.WARN,
        detail=f"server v{live} differs from pinned v{expected}",
        hint="Re-probe the API surface for new endpoints, then update WDGOWARS_EXPECTED_VERSION",
    )
...
async def run_all() -> list[CheckResult]:
    ...
    wdg_ver_task = asyncio.create_task(check_wdgowars_version())
    ors_task = asyncio.create_task(check_ors())
    api_results = await asyncio.gather(wigle_task, wdg_task, wdg_ver_task, ors_task)
