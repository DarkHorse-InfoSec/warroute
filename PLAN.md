...
**Project name:** WarRoute
**Owner:** @DarkHorse-InfoSec
**Status:** SHIPPED + deployed. v1 (all phases below) plus post-v1 work is live at
`https://warroute.darkhorseinfosec.com`. In tester beta; repo being prepared for public open-source.

> **Post-v1 note (2026-07-04):** the design below is preserved as the canonical spec. Several things
> evolved after v1 and are documented in [`DECISIONS.md`](./DECISIONS.md) + [`README.md`](./README.md),
> not by rewriting this file: multi-user tester access + per-user home/API-credentials (still auth-at-edge,
> no app user model); a per-user choice of navigation app (Google Maps, Apple Maps, Waze, device-default
> `geo:`, GPX for OsmAnd/Organic Maps) rather than Google-Maps-only; an optional Cloudflare Access auth
> mode alongside basic_auth; a gang-territory overlay on `/coverage`; and multi-stop / arrival-time /
> roadtrip planner extensions (Phase 6 series). Where this spec says "Google Maps only" or "single-user",
> read it as the original intent; current behavior is in DECISIONS.md.
...
- No multi-user. Single-tenant for the maintainer's account.
...
- **Dev:** local checkout (developed on Windows + WSL2, but any POSIX-ish environment works).
...
- **HTMX over React:** mobile-first, no bundler, no `npm install` hell. the maintainer has explicitly identified marketing/sales - not frontend complexity - as a weak point. Don't add weight.
...
- `LICENSE` - MIT or proprietary, the maintainer's call. Default to **proprietary "All rights reserved"** for v1 since it touches DarkHorse infra.
- Project guidelines doc at repo root: code style, never commit secrets, all secrets in `.env`, prod paths, dev paths.
...
- The planned route, when driven, produces a CSV that scores ≥3× more new APs per minute than a same-duration drive on the maintainer's normal commute. (Validate empirically on a real drive.)
...
- Watcher detects new run finished → push notification to phone (via existing DarkHorse Twilio account or ntfy.sh - the maintainer's call).
...
Spin up a **fresh CPX21** on its own box (don't co-host on an unrelated VPS - different security posture and uptime requirements).

```
Type:     CPX21 (3 vCPU, 4 GB RAM, 80 GB SSD) - €8.46/mo
Image:    Debian 12
Location: Falkenstein or Helsinki (latency to VT is similar; pick on price)
SSH key:  your existing deploy public key
Hostname: warroute
Firewall: 22 (SSH, your IPs only), 80, 443 (Caddy)
```

**Initial setup script** (`infra/bootstrap.sh`):
...
These were the open questions; the maintainer resolved them as follows.
...
Original plan was self-hosted OSRM with a regional OSM extract (VT + NH + QC). the maintainer wants WarRoute usable anywhere in the world, not just New England. A worldwide OSM extract is ~120 GB on disk and needs ~32 GB RAM to serve - a CPX21 cannot host it.
...
This also means the Hetzner box can be a **CPX11** instead of CPX21 (1 vCPU, 2 GB RAM, €4.51/mo). the maintainer already provisioned at `5.161.250.8` - keep whatever was provisioned, just don't bother with the OSRM piece.
...
Original plan proposed `score = 0.6 × new_to_you + 0.4 × new_territory_cell`. the maintainer prefers WarRoute to be a thin orchestration layer over the existing services rather than a competing scoring system.
...
the maintainer must obtain `WIGLE_API_TOKEN` and `WDGWARS_API_KEY` manually before Phase 1 testing; this is a manual step.
...
1. **the maintainer:** sign up at https://wdgwars.pl, generate API key, save it. Confirm your WiGLE.net API token from `wigle.net/account`. Provision the Hetzner CPX21 and set up DNS for `warroute.darkhorseinfosec.com`.
2. **Open a fresh build session** in your local `warroute` checkout.
3. **First instruction to the build session:**
...
5. **When the build hits a real architectural fork** (something not covered in this plan, or where the plan turns out to be wrong), write the question + a proposed answer to `DECISIONS.md` and ping the maintainer for sign-off before changing direction.
...
*End of plan. Total expected build time: 15-20 hours of build wall-clock, spread across 4-7 sessions. Phase 1 alone delivers most of the day-to-day value; Phase 3 delivers the magic.*
