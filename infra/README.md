...
**This runbook is for a human operator.** The files in this dir can be generated ahead of time, but these steps must not be executed on the production VPS without explicit per-step confirmation. See `DECISIONS.md` for the safety boundary.
...
- [ ] You have SSH access to the VPS as root or a sudoer (via your configured SSH key).
- [ ] You have the WiGLE, WDGoWars, ORS, and optional Mapbox tokens in hand. Do NOT paste tokens into any chat or AI assistant.
...
- `NTFY_TOPIC=` (optional, e.g. `warroute-admin-<random>`)
...
# Expect 200:
curl -I -u admin:<YOUR_PASSWORD> https://warroute.darkhorseinfosec.com
...
The operator account `admin` is protected: `remove-tester.sh` refuses to delete it. To rotate `admin`'s password, edit `/etc/caddy/Caddyfile` manually with a hash from `caddy hash-password`, then `systemctl reload caddy`.

---

## Auth modes

WarRoute is auth-unaware by design: no login form, no JWT, no session inside the
app. Since the stateless access model (DECISIONS.md 2026-07-04), each user brings
their own WiGLE / WDGoWars / ORS keys, held in their browser and sent as X-*
headers; the server stores nothing. That means the edge gate is now optional: it
controls only WHO can reach the instance, not how keys are handled. Pick a mode by
which Caddyfile you install to `/etc/caddy/Caddyfile`.

### Mode 0: public / no gate (stateless model)

Config file: `infra/Caddyfile.public`. No edge auth. Anyone can open the app and
use it with their own keys. Safe because no useful operation runs without the
caller's own keys, and the one shared capability (ORS routing) is guarded by the
app's per-IP rate limit + per-day cap (`ORS_SHARED_*`). This is the intended
posture for a public open beta. Modes 1 and 2 below keep the beta CLOSED to a
known set of people; the app itself is identical under all three.

### Mode 1: basic_auth (closed beta)

Config file: `infra/Caddyfile`. Zero external dependencies. Users are HTTP
basic-auth accounts managed with the helper scripts:

```bash
warroute-add-tester alice      # generate password, bcrypt, append to Caddyfile, reload
warroute-remove-tester alice   # strip the line, delete the plaintext file, reload
```

See section 5 (install the Caddyfile + operator account) and section 7a (add and
revoke tester accounts) above for the full procedure. This is the recommended
default for small deployments.

### Mode 2: Cloudflare Access SSO (optional)

Config file: `infra/Caddyfile.cloudflare` (draft; review before deploying).
Cloudflare Access provides Google or email login at Cloudflare's edge, free up to
50 users. Cloudflare authenticates the user and forwards their identity in the
`Cf-Access-Authenticated-User-Email` request header; Caddy maps that header into
`X-Forwarded-User`, so the app behaves identically to the basic_auth path.

**Critical security requirement: lock the origin to Cloudflare only.**
Caddy core cannot validate the Cloudflare Access JWT without a plugin, so the
security control for this mode is a network origin lock, not signature
verification. The origin server MUST be reachable only from Cloudflare. Set the
Hetzner Cloud Firewall so inbound 443 is allowed only from Cloudflare's published
IP ranges:

- https://www.cloudflare.com/ips/

If the origin is reachable directly from any IP, an attacker can send a forged
`Cf-Access-Authenticated-User-Email` header straight to the origin and impersonate
any user. There is no softer version of this rule. Before deploying this variant,
verify the lock by trying to reach the origin IP on 443 from an off-Cloudflare
host; the connection must be refused. For defense in depth, additionally validate
the Cloudflare Access JWT with a Caddy plugin, but the firewall allowlist is the
baseline non-negotiable control.

To switch to this mode:

```bash
# Fill in your Cloudflare Access application + policy first (Cloudflare dashboard),
# then lock the Hetzner Cloud Firewall to Cloudflare's IP ranges on 443.
cp /home/warroute/warroute/infra/Caddyfile.cloudflare /etc/caddy/Caddyfile
...
```bash
# No `warroute` host alias is configured yet, so connect with the file-based key
# directly (the box key is NOT in the 1Password agent). From git-bash on Windows,
# prefix MSYS_NO_PATHCONV=1 so the remote unix paths are not mangled:
MSYS_NO_PATHCONV=1 ssh -i "~/.ssh/hetzner-warroute" -o IdentitiesOnly=yes root@5.161.250.8
...
Migrations run automatically on app startup (the FastAPI lifespan handler calls
`run_migrations`, so every new `_vN.sql` applies on `systemctl restart warroute`);
no manual SQL needed for additive schema changes. Confirm in the log:
`journalctl -u warroute --since '30 seconds ago' | grep -i migration`.

Note: origin must have the code first. If the 1Password SSH agent is failing and
`git push` over the SSH remote dies with "communication with agent failed", push
over HTTPS instead: `git push https://github.com/DarkHorse-InfoSec/warroute.git main`.
