...
def fixed_now(monkeypatch: pytest.MonkeyPatch) -> datetime:
    """Freeze planner._now() at 9am on the test date so deadline-feasibility tests
    don't depend on wall-clock time. Tests that want a different "now" can
    monkeypatch again inside their body."""
    fake = datetime(2026, 5, 14, 9, 0)
    monkeypatch.setattr("warroute.router.planner._now", lambda: fake)
    return fake
...
@respx.mock
async def test_plan_with_arrive_by_computes_departure(fixed_now: datetime) -> None:
...
# ----------------------------------------------------------------------------
# Phase 6b.3: per-stop arrive_by deadlines
# ----------------------------------------------------------------------------


def test_parse_arrive_hhmm_today() -> None:
    now = datetime(2026, 5, 14, 10, 0)
    out = parse_arrive_hhmm("1600", now=now)
    assert out == datetime(2026, 5, 14, 16, 0)


def test_parse_arrive_hhmm_rolls_to_tomorrow_when_past() -> None:
    now = datetime(2026, 5, 14, 17, 0)  # 5pm; 4pm already past
    out = parse_arrive_hhmm("1600", now=now)
    assert out == datetime(2026, 5, 15, 16, 0)


def test_parse_arrive_hhmm_accepts_colon_form() -> None:
    now = datetime(2026, 5, 14, 10, 0)
    assert parse_arrive_hhmm("16:00", now=now) == datetime(2026, 5, 14, 16, 0)
    assert parse_arrive_hhmm(" 16:00 ", now=now) == datetime(2026, 5, 14, 16, 0)


def test_parse_arrive_hhmm_rejects_garbage() -> None:
    now = datetime(2026, 5, 14, 10, 0)
    assert parse_arrive_hhmm("", now=now) is None
    assert parse_arrive_hhmm(None, now=now) is None
    assert parse_arrive_hhmm("nope", now=now) is None
    assert parse_arrive_hhmm("2500", now=now) is None  # hour out of range
    assert parse_arrive_hhmm("1265", now=now) is None  # minute out of range
    assert parse_arrive_hhmm("160", now=now) is None  # too short


@respx.mock
async def test_per_stop_arrive_by_binds_departure_multistop(fixed_now: datetime) -> None:
    """A per-stop deadline determines departure when tighter than the trip end.

    Layout: home -> A (no deadline) -> B (arrive_by 4pm) -> home (loop).
    Per-segment mock duration = 10 min. dwell at A = 5 min, B = 0.
    Offset to B = 10 (home->A drive) + 5 (A dwell) + 10 (A->B drive) = 25 min.
    Departure = 4pm - 25 - 2 buffer = 3:33pm.
    """
    run_migrations()
    _seed_scored_grid(44.94, -72.21, radius_km=8)
    respx.post(ORS_API_BASE + OPTIMIZATION_PATH).mock(
    ...
    respx.post(ORS_API_BASE + DIRECTIONS_PATH).mock(
        return_value=httpx.Response(
            200,
            json={"routes": [{"summary": {"distance": 15000, "duration": 1800}, "geometry": None}]},
        )
    )

    arrive_b = datetime(2026, 5, 14, 16, 0)  # 4pm
    req = PlanRequest(
        home_lat=44.94,
        home_lon=-72.21,
        duration_min=120,
        ...
        stops=[
            Stop(lat=44.95, lon=-72.20, label="A", dwell_min=5),
            Stop(lat=44.96, lon=-72.19, label="B (pickup)", arrive_by=arrive_b),
        ],
    )
    result = await plan(req)

    assert result.binding_stop_index == 1
    assert result.departure_at == arrive_b - timedelta(minutes=25 + 2)
    assert len(result.stop_arrivals) == 2
    # ETA at B should equal arrive_by - buffer (we leave with 2 min slack).
    assert result.stop_arrivals[1] == arrive_b - timedelta(minutes=2)


@respx.mock
async def test_per_stop_earliest_binding_wins(fixed_now: datetime) -> None:
    """When multiple stops have arrive_by, the one needing earliest departure wins."""
    run_migrations()
    _seed_scored_grid(44.94, -72.21, radius_km=8)
    respx.post(ORS_API_BASE + OPTIMIZATION_PATH).mock(
        return_value=httpx.Response(
    ...
    # Stop A at 1pm, Stop B at 5pm. Drive home->A = 10 min, A->B = 10 min.
    # A's required departure: 13:00 - 10 - 2 = 12:48
    # B's required departure: 17:00 - (10 + 0 + 10) - 2 = 16:38
    # A is tighter -> binding_stop_index = 0
    req = PlanRequest(
        home_lat=44.94,
        home_lon=-72.21,
        duration_min=180,
        mode="loop",
        stops=[
            Stop(lat=44.95, lon=-72.20, label="A", arrive_by=datetime(2026, 5, 14, 13, 0)),
            Stop(lat=44.96, lon=-72.19, label="B", arrive_by=datetime(2026, 5, 14, 17, 0)),
        ],
    )
    result = await plan(req)
    assert result.binding_stop_index == 0
    assert result.departure_at == datetime(2026, 5, 14, 13, 0) - timedelta(minutes=12)


@respx.mock
async def test_per_stop_deadline_drops_cells_when_infeasible(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When departure would require leaving in the past, prefix cells are dropped
    in favor of direct legs, and deadline_drops counts the cells removed."""
    # _now = 8:30am; deadline arrive_by = 9:00am at stop B (after stop A).
    # With cells, segment durations are 20 min each (40+ min total) -> infeasible.
    # After dropping cells, direct legs are 5 min each (15 min total) -> feasible.
    monkeypatch.setattr("warroute.router.planner._now", lambda: datetime(2026, 5, 14, 8, 30))
    run_migrations()
    _seed_scored_grid(44.94, -72.21, radius_km=8)

    # First call (with cells) returns 20 min. Subsequent /optimization calls also 20 min.
    # /directions returns 5 min (used for both retry-direct legs AND full-chain).
    respx.post(ORS_API_BASE + OPTIMIZATION_PATH).mock(
        return_value=httpx.Response(
            200,
            json={
                "routes": [
                    {
                        "vehicle": 1,
                        "duration": 1200,  # 20 min/segment with cells
                        "distance": 8000,
                        "steps": [{"type": "job", "job": 0}],
                    }
                ]
    ...
    respx.post(ORS_API_BASE + DIRECTIONS_PATH).mock(
        return_value=httpx.Response(
            200,
            json={"routes": [{"summary": {"distance": 3000, "duration": 300}, "geometry": None}]},
        )
    )
    ...
    req = PlanRequest(
        ...
        mode="loop",
        stops=[
            Stop(lat=44.95, lon=-72.20, label="A", dwell_min=0),
            Stop(
                lat=44.96,
                lon=-72.19,
                label="B (pickup)",
                arrive_by=datetime(2026, 5, 14, 9, 0),
            ),
        ],
    )
    result = await plan(req)

    # Cells were dropped from prefix (segments 0 and 1 leading to stop B).
    assert result.deadline_drops > 0
    # Departure should now be feasible (>= 8:30 + 5 min lead).
    assert result.departure_at is not None
    assert result.departure_at >= datetime(2026, 5, 14, 8, 35)


@respx.mock
async def test_per_stop_deadline_raises_when_truly_infeasible(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When even direct driving can't meet the deadline, planner raises with the
    specific stop named and the shortfall in minutes."""
    # _now = 8:55am; deadline = 9:00am. Direct drive is 30 min (mocked /directions).
    # Even without cells, we'd need to leave at 8:28am to make 9:00.
    monkeypatch.setattr("warroute.router.planner._now", lambda: datetime(2026, 5, 14, 8, 55))
    run_migrations()
    _seed_scored_grid(44.94, -72.21, radius_km=8)
        ...
        return_value=httpx.Response(
            ...
            json={
                "routes": [
                    {
                        "vehicle": 1,
                        "duration": 1800,  # 30 min, with or without cells
                        "distance": 20000,
                        "steps": [{"type": "job", "job": 0}],
                    }
                ]
    ...
    respx.post(ORS_API_BASE + DIRECTIONS_PATH).mock(
        return_value=httpx.Response(
            200,
            json={"routes": [{"summary": {"distance": 20000, "duration": 1800}, "geometry": None}]},
        )
    )
    ...
    with pytest.raises(PlannerError, match="School \\(pickup\\)"):
        await plan(req)


@respx.mock
async def test_stops_json_roundtrips_arrive_by(fixed_now: datetime) -> None:
    """The stops_json column should persist arrive_by as ISO datetime."""
    run_migrations()
    _seed_scored_grid(44.94, -72.21, radius_km=8)
    respx.post(ORS_API_BASE + OPTIMIZATION_PATH).mock(
        return_value=httpx.Response(
    ...
    respx.post(ORS_API_BASE + DIRECTIONS_PATH).mock(
        return_value=httpx.Response(
            200,
            json={"routes": [{"summary": {"distance": 12000, "duration": 1500}, "geometry": None}]},
        )
    )

    arrive = datetime(2026, 5, 14, 16, 0)
    req = PlanRequest(
        home_lat=44.94,
        home_lon=-72.21,
        duration_min=120,
        mode="loop",
        stops=[
            Stop(lat=44.95, lon=-72.20, label="A"),
            Stop(lat=44.96, lon=-72.19, label="B", arrive_by=arrive),
        ],
    )
    result = await plan(req)
    ...
    with transaction() as conn:
        row = conn.execute(
            "SELECT stops_json FROM planned_routes WHERE id = ?",
            (result.planned_route_id,),
        ).fetchone()
    persisted = json.loads(row["stops_json"])
    assert persisted[0]["arrive_by"] is None
    assert persisted[1]["arrive_by"] == arrive.isoformat()


@respx.mock
async def test_per_stop_arrive_by_single_segment_oneway(fixed_now: datetime) -> None:
    """Oneway + 1 stop with arrive_by goes through the single-segment plan() path.

    The stop's arrive_by should compute departure the same way the request-level
    arrive_by does today (the single stop == the trip end).
    """
    run_migrations()
    _seed_scored_grid(44.9367, -72.2051, radius_km=4)
    respx.post(ORS_API_BASE + OPTIMIZATION_PATH).mock(
    ...
    arrive = datetime(2026, 5, 14, 17, 0)
    req = PlanRequest(
        home_lat=44.9367,
        home_lon=-72.2051,
        duration_min=90,
        mode="oneway",
        stops=[Stop(lat=44.96, lon=-72.19, label="Destination", arrive_by=arrive)],
    )
    result = await plan(req)
    assert result.binding_stop_index == 0
    # 30 min drive + 2 min buffer = leave 32 min before 5pm
    assert result.departure_at == arrive - timedelta(minutes=32)
