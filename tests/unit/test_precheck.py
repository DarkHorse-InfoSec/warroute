...
from warroute.clients.wdgowars import ME_PATH, STATS_PATH, WDGOWARS_API_BASE
...
async def test_run_all_happy_path() -> None:
    ...
    respx.get(WDGOWARS_API_BASE + STATS_PATH).mock(
        return_value=httpx.Response(200, json={"version": "1.3.0"})
    )
    respx.post(ORS_API_BASE + DIRECTIONS_PATH).mock(
        return_value=httpx.Response(
            200,
...
# --- WDGoWars server-version awareness (post-v1) ---


@respx.mock
async def test_check_wdgowars_version_ok_when_matches(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WDGOWARS_EXPECTED_VERSION", "1.3.0")
    get_settings.cache_clear()
    respx.get(WDGOWARS_API_BASE + STATS_PATH).mock(
        return_value=httpx.Response(200, json={"version": "1.3.0"})
    )
    result = await check_wdgowars_version()
    assert result.status == Status.OK
    assert "1.3.0" in result.detail


@respx.mock
async def test_check_wdgowars_version_warns_on_mismatch(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WDGOWARS_EXPECTED_VERSION", "1.3.0")
    get_settings.cache_clear()
    respx.get(WDGOWARS_API_BASE + STATS_PATH).mock(
        return_value=httpx.Response(200, json={"version": "1.4.0"})
    )
    result = await check_wdgowars_version()
    assert result.status == Status.WARN
...
@respx.mock
async def test_check_wdgowars_version_warns_when_stats_errors() -> None:
    respx.get(WDGOWARS_API_BASE + STATS_PATH).mock(return_value=httpx.Response(500))
    result = await check_wdgowars_version()
    assert result.status == Status.WARN


@respx.mock
async def test_check_wdgowars_version_warns_when_field_missing() -> None:
    respx.get(WDGOWARS_API_BASE + STATS_PATH).mock(
        return_value=httpx.Response(200, json={"uptime": 1})
    )
    result = await check_wdgowars_version()
    assert result.status == Status.WARN
