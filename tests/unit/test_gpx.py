...
# --- Single-destination nav hand-offs: target the first stop after origin ---

_LOOP = [
    Waypoint(44.94, -72.21, label="Home"),  # 0 origin
    Waypoint(44.96, -72.18, label="Cell A"),  # 1 first stop
    Waypoint(44.95, -72.19, label="Cell B"),  # 2
    Waypoint(44.94, -72.21, label="Home"),  # 3 back home
]


def test_apple_maps_url_targets_first_stop() -> None:
    url = apple_maps_url(_LOOP)
    assert url.startswith("https://maps.apple.com/?")
    qs = urllib.parse.parse_qs(urllib.parse.urlparse(url).query)
    assert qs["daddr"] == ["44.96,-72.18"]
    assert qs["dirflg"] == ["d"]
    # No saddr: Apple Maps routes from the driver's current location.
    assert "saddr" not in qs


def test_waze_url_targets_first_stop_and_navigates() -> None:
    url = waze_url(_LOOP)
    assert url.startswith("https://waze.com/ul?")
    qs = urllib.parse.parse_qs(urllib.parse.urlparse(url).query)
    assert qs["ll"] == ["44.96,-72.18"]
    assert qs["navigate"] == ["yes"]


def test_geo_uri_targets_first_stop_with_label() -> None:
    uri = geo_uri(_LOOP, label="WarRoute start")
    assert uri.startswith("geo:44.96,-72.18?q=")
    # The query is percent-encoded "lat,lon(label)".
    assert "44.96%2C-72.18" in uri
    assert "WarRoute%20start" in uri


def test_two_point_plan_targets_the_destination() -> None:
    # With only origin+destination, the "first stop" is the destination itself.
    pair = [Waypoint(44.94, -72.21), Waypoint(45.00, -72.00)]
    qs = urllib.parse.parse_qs(urllib.parse.urlparse(waze_url(pair)).query)
    assert qs["ll"] == ["45.0,-72.0"]


@pytest.mark.parametrize("builder", [apple_maps_url, waze_url, geo_uri])
def test_single_dest_builders_require_two_points(builder: object) -> None:
    with pytest.raises(ValueError):
        builder([Waypoint(44.94, -72.21)])  # type: ignore[operator]
